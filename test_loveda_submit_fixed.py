...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate LoveDA test-set prediction masks for submission (0..6 labels)"
    )
    parser.add_argument("--config", type=str, required=True, help="Path to YAML config file")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to trained checkpoint (.pth)")
    parser.add_argument("--test_dir", type=str, required=True, help="LoveDA test image root")
    ...
    parser.add_argument("--recursive", action="store_true", help="Recursively search images and preserve subfolder structure")
    parser.add_argument(
        "--exts",
        type=str,
        nargs="+",
    ...
    parser.add_argument("--save_zip", action="store_true", help="Zip output_dir after masks are generated")
...
def choose_device(cfg_device: str, override_device: str = ""):
    name = override_device if override_device else cfg_device
...
def pred_to_submission_label(pred: np.ndarray) -> np.ndarray:
    """
    LoveDA website evaluation expects class IDs 0..6 in submission masks.
    Therefore do NOT shift by +1.
    """
...
def main():
    ...
    with torch.no_grad():
        for idx, img_path in enumerate(image_paths, start=1):
            ...
            submission_mask = pred_to_submission_label(pred)
            ...
            save_mask(submission_mask, save_path)
